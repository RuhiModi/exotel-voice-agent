/*************************************************
 * GUJARATI AI VOICE AGENT (SHEET + WHATSAPP)
 * Stable Twilio Gather + Gujarati
 * Conversational Sheet + WhatsApp/SMS Summary
 *************************************************/
...
function buildSummary(call) {
  if (call.result === "task_done")
    return "User confirmed work completed and expressed satisfaction.";
  if (call.result === "problem_recorded")
    return `User reported pending work. Details: ${call.userTexts.join(" ")}`;
  if (call.result === "end_no_time")
    return "User was unavailable and requested follow-up later.";
...
/* ======================
   WHATSAPP / SMS
====================== */
function sendWhatsAppSummary(call) {
  const message = `
📞 કોલ સારાંશ

સ્થિતિ: ${call.result}
વિગત: ${call.userTexts.join(" ")}
...
app.listen(PORT, async () => {
  await preloadAll();
  console.log("✅ Gujarati AI Voice Agent running (Sheet + WhatsApp enabled)");
});
